...
public class Light_On_Player : MonoBehaviour 
{
    ...
    void Start()
    {
		...
		var v3Post = LightLocation.transform.position;
		v3Post.z = 0;
		CreateLight (Vector3.zero);
//		CreateLight(LightLocation.transform.position + new Vector3(0, 0, 10));
    }
    ...
    void OnPostRender()
    {
        ...
        foreach (Light2D l in lightsInScene)
        {
            ...
            {
				GL.Vertex(LightLocation.transform.position + l.transform.position + (Vector3)circleLookup[i - 1]);
				GL.Vertex(LightLocation.transform.position + l.transform.position + (Vector3)circleLookup[i]);
            }

			GL.Vertex(LightLocation.transform.position + l.transform.position + (Vector3)circleLookup[circleLookup.Length - 1]);
			GL.Vertex(LightLocation.transform.position + l.transform.position + (Vector3)circleLookup[0]);
